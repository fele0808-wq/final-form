...
import { DAY_END, DAY_START, buildPlanFromPrompt, findPlanOverlaps, findScheduleConflict, formatPlannerDuration, formatPlannerTime, getFreeBlocks, getPlanIdentity, splitPromptTasks, type Activity, type PlanOverlap, type PlannedItem, type PlanningAnchor, type ScheduleConflict } from '@/lib/planner';
import { applyPlanningPreferences, getPlanningContext, loadPlans, loadPlanningPreferences, rememberMovedPlan, rememberPlan, savePlans, type PlanningPreferences } from '@/lib/planning-memory';
...
export default function PlanScreen() {
  ...
  const [pendingScheduleTasks, setPendingScheduleTasks] = useState<string[]>([]);
  const [pendingAmbiguousTasks, setPendingAmbiguousTasks] = useState<string[]>([]);
  const [pendingScheduleIndex, setPendingScheduleIndex] = useState(0);
  const [pendingScheduleTotal, setPendingScheduleTotal] = useState(0);
  ...
  useEffect(() => {
    loadPlans().then((saved) => {
      if (saved.plannedItems.length) setPlannedItems(saved.plannedItems);
  ...
  useEffect(() => {
    ...
    void savePlans(plannedItems, activities);
  }, [plannedItems, activities]);
  ...
  const submitPrompt = () => {
    ...
    const taskPrompts = splitPromptTasks(nextPrompt);
    const hasExplicitTime = (taskPrompt: string) => /\b\d{1,2}(?::\d{2})?\s*(?:am|pm)\b|\b\d{1,2}:\d{2}\b|\b(?:noon|midnight)\b/i.test(taskPrompt);
    if (taskPrompts.length > 1) {
      const untimedTasks = taskPrompts.filter((taskPrompt) => !hasExplicitTime(taskPrompt) && !findNextAmbiguousTime(taskPrompt));
      const ambiguousTasks = taskPrompts.filter((taskPrompt) => !hasExplicitTime(taskPrompt) && findNextAmbiguousTime(taskPrompt));
      taskPrompts.filter(hasExplicitTime).forEach((taskPrompt) => completePrompt(taskPrompt));
      if (ambiguousTasks.length) {
        const ambiguousTask = ambiguousTasks[0];
        setPendingAmbiguousTasks(ambiguousTasks.slice(1));
        setPendingScheduleTasks(untimedTasks);
        setPendingScheduleIndex(0);
        setPendingScheduleTotal(untimedTasks.length);
        setPendingPrompt(ambiguousTask);
        setPendingTime(findNextAmbiguousTime(ambiguousTask));
      ...
      if (untimedTasks.length) {
        ...
        setPendingScheduleIndex(0);
        setPendingScheduleTotal(untimedTasks.length);
        setPendingSchedulePrompt(untimedTasks[0]);
    ...
    if (ambiguousTime) {
      ...
      setPendingAmbiguousTasks([]);
      setActiveModal('timePeriod');
      return;
    }
    if (!hasExplicitTime(nextPrompt)) {
      const taskPrompts = splitPromptTasks(nextPrompt);
      setPendingScheduleTasks(taskPrompts.length > 1 ? taskPrompts : []);
      setPendingAmbiguousTasks([]);
      setPendingScheduleIndex(0);
      setPendingScheduleTotal(taskPrompts.length);
      setPendingSchedulePrompt(taskPrompts[0] ?? nextPrompt);
      setScheduleRepeats(false);
      setScheduleDays([1, 2, 3, 4, 5]);
      setScheduleRepeatWeeks(4);
      setActiveModal('scheduleWindow');
  ...
  const submitScheduleWindow = () => {
    ...
    const scheduledPrompt = `${pendingSchedulePrompt} from ${formatPlannerTime(scheduleStartMinutes)} to ${formatPlannerTime(scheduleEndMinutes)}`;
    completePrompt(scheduledPrompt, false, scheduleRepeats ? scheduleDays : undefined, scheduleRepeats ? scheduleRepeatWeeks : undefined);
    const remainingTasks = pendingScheduleTasks.slice(1);
    if (remainingTasks.length) {
      setPendingScheduleTasks(remainingTasks);
      setPendingScheduleIndex((current) => current + 1);
      setPendingSchedulePrompt(remainingTasks[0]);
      setScheduleStartMinutes(9 * 60);
      setScheduleEndMinutes(10 * 60);
      setScheduleRepeats(false);
      setActiveModal('scheduleWindow');
    } else {
      setPendingScheduleTasks([]);
      setPendingScheduleIndex(0);
      setPendingScheduleTotal(0);
  ...
  const chooseTimePeriod = (period: 'AM' | 'PM') => {
    ...
    if (pendingAmbiguousTasks.length) {
      completePrompt(clarifiedPrompt);
      const nextTask = pendingAmbiguousTasks[0];
      setPendingAmbiguousTasks(pendingAmbiguousTasks.slice(1));
      setPendingPrompt(nextTask);
      setPendingTime(findNextAmbiguousTime(nextTask));
      setActiveModal('timePeriod');
      return;
    }
    completePrompt(clarifiedPrompt);
    if (pendingScheduleTasks.length) {
      setPendingSchedulePrompt(pendingScheduleTasks[0]);
      setActiveModal('scheduleWindow');
    }
  };
    ...
    <ThemedView style={styles.container}>
      ...
      <SwipeSheet
        ...
        <View style={styles.calendarSheet}>
          ...
          <ThemedText type="small" themeColor="textSecondary" style={styles.timePeriodPrompt}>
            {pendingScheduleTotal > 1 ? `Task ${pendingScheduleIndex + 1} of ${pendingScheduleTotal}: ` : ''}I can plan “{pendingSchedulePrompt}”. Choose a start and end time so I can place it around your other plans.
          </ThemedText>
          <Pressable
