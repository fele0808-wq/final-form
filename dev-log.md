...
- Sequential multi-task prompts split only on explicit markers such as `then`, `after that`, `and then`, or `;`; bare commas and `and` remain part of one activity name.
- Confirmed plans and calendar activities persist locally under `final-form-planning-plans-v1` and are restored when the planning screen launches again; dates are stored as local calendar days.
...
- `src/lib/planning-memory.ts`: local preference, plan, and activity persistence and learned defaults.
...
- The planning screen owns the live schedule state; confirmed plans and calendar activities are persisted locally, while external calendar-provider synchronization remains future work.
- Learned preferences, confirmed plans, and calendar activities persist locally under versioned AsyncStorage keys, including `final-form-planning-plans-v1`.
